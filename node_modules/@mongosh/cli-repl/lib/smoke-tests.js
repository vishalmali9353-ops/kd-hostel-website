    ...
    for (const { name, tags, input, output, env, testArgs, includeStderr, exitCode, perfTestIterations, } of [
      ...
      db.dropDatabase();`,
            ...
            },
            {
                ...
                input: `let count = 0; for (const item of ${manyDocsCursor(200_000)}) count++; print(count);`,
...
async function resetTelemetryThrottleState() {
    const dir = (0, config_directory_1.getStoragePaths)().shellLocalDataPath;
    let entries = [];
    try {
    ...
    await Promise.all(entries
        ...
        .map(async (name) => {
        try {
            await fs_1.promises.unlink(path_1.default.join(dir, name));
        }
        catch {
        }
    }));
