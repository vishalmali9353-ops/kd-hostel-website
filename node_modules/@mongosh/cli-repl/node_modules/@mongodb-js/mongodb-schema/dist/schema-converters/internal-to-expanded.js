...
const internal_to_standard_1 = require("./internal-to-standard");
const internal_to_mongodb_1 = require("./internal-to-mongodb");
const util_1 = require("../util");
const createConvertInternalToExpanded = function () {
    const usedDefinitions = new Set();
    function getUsedDefinitions() {
        const filteredDefinitions = Object.fromEntries(Object.entries(internal_to_standard_1.RELAXED_EJSON_DEFINITIONS).filter(([key]) => usedDefinitions.has(key)));
    ...
    function getStandardType(internalType) {
        const type = internal_to_standard_1.InternalTypeToStandardTypeMap[internalType];
        if (!type)
            throw new Error(`Encountered unknown type: ${internalType}`);
        return { ...type };
    }
    function getBsonType(internalType) {
        const type = internal_to_mongodb_1.InternalTypeToBsonTypeMap[internalType];
        if (!type)
            throw new Error(`Encountered unknown type: ${internalType}`);
        return type;
    }
...
//# sourceMappingURL=internal-to-expanded.js.map