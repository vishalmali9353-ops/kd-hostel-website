...
class MongoshNodeRepl {
    ...
    async finishInitializingNodeRepl() {
        ...
        const origReplCompleter = await (0, node_repl_fix_completer_side_effects_1.fixNodeReplCompleterSideEffectHandling)((0, node_repl_fix_completer_side_effects_1.promisifyCompleter)(repl.completer.bind(repl)));
    ...
    async greet(mongodVersion, greeting) {
        ...
        if (greeting?.moreRecentMongoshVersion) {
            if (greeting?.isHomebrew) {
                text += `mongosh ${this.clr(greeting.moreRecentMongoshVersion, 'bold')} is available, run ${this.clr('brew upgrade mongosh', 'mongosh:uri')} to update\n`;
            }
            else {
