...
const setup_analytics_1 = require("./setup-analytics");
const types_1 = require("@mongosh/types");
const fs_1 = require("fs");
const path_1 = __importDefault(require("path"));
const get_os_info_1 = require("@mongodb-js/get-os-info");
...
class CliRepl {
    constructor(options) {
        ...
        this.configDirectory = new config_directory_1.ConfigManager(this.shellHomeDirectory)
            ...
            .on('new-config', () => {
            this.setTelemetryEnabled().catch((err) => {
        ...
        this.updateNotificationManager = new update_notification_manager_1.UpdateNotificationManager({
            ...
            onInvalidMatchPattern: (err) => this.bus.emit('mongosh:error', err, 'cta'),
        });
    ...
    async _start(driverUri, driverOptions) {
        ...
        const buildInfoPromise = (0, build_info_1.buildInfo)();
        const [moreRecentMongoshVersion, currentVersionCTA, { installationMethod },] = await Promise.all([
            this.getMoreRecentMongoshVersion(),
            buildInfoPromise.then((info) => this.updateNotificationManager.getGreetingCTAForCurrentVersion(info)),
            buildInfoPromise,
        ]);
    ...
    async setupTelemetrySink() {
        const { analytics, telemetryEndpoint } = await (0, setup_analytics_1.resolveToggleableAnalytics)({
    ...
    async isTelemetryEnabled() {
        return (!this.forceDisableTelemetry && !!(await this.getConfig('enableTelemetry')));
    }
    async setTelemetryEnabled() {
    ...
    async loadGlobalConfigFile() {
        ...
        try {
            ...
            if (fileContents.trim().startsWith('{')) {
                ...
                config = fileContents
                    ? yaml.load(fileContents)?.mongosh ?? {}
                    : {};
            }
    ...
    async close() {
        return (this.closingPromise ??= (async () => {
            ...
            this.logWriter?.info('MONGOSH', (0, mongodb_log_writer_1.mongoLogId)(1_000_000_045), 'analytics', 'Persisted telemetry throttle state', {
                flushError,
                flushDuration,
            });
