/* ---------------- Page navigation ---------------- */
  ...
  /* ---------------- Student Database (50 seeded records - Gujarat) ----------------
     Names/rooms below are generated from the same firstNames/lastNames/blocks lists
     used in mess.js, so every student (incl. girls) matches the Mess Management
     records — the "Meeting Student" field here always resolves to the same name. */
  ...
  /* ---------------- ID Proof Number: type-aware input rules ---------------- */
  const idProofRules = {
    'Aadhaar Card':      { maxlength: 12, mode: 'numeric', placeholder: '12-digit Aadhaar number', hint: 'Enter the 12-digit Aadhaar number.' },
    'College ID':        { maxlength: 15, mode: 'alnum',   placeholder: 'e.g. KDP1023',            hint: 'Enter a valid College ID (letters & numbers).' },
    'Driving License':   { maxlength: 16, mode: 'alnum',   placeholder: 'e.g. GJ0120230012345',     hint: 'Enter a valid Driving License number.' },
    'Voter ID':          { maxlength: 10, mode: 'alnum',   placeholder: 'e.g. ABC1234567',           hint: 'Enter a valid Voter ID (EPIC) number.' },
    'Passport':          { maxlength: 8,  mode: 'alnum',   placeholder: 'e.g. M1234567',             hint: 'Enter a valid Passport number.' },
    'Other':             { maxlength: 20, mode: 'alnum',   placeholder: 'ID number',                 hint: 'Enter the ID proof number.' }
  };

  const vIdProof = $('#vIdProof');
  const vIdProofNo = $('#vIdProofNo');
  const errIdProofNo = $('#err-vIdProofNo');

  vIdProof.addEventListener('change', () => {
    const rule = idProofRules[vIdProof.value];
    if(rule){
      vIdProofNo.disabled = false;
      vIdProofNo.placeholder = rule.placeholder;
      vIdProofNo.maxLength = rule.maxlength;
      vIdProofNo.value = '';
      errIdProofNo.innerHTML = `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> ${rule.hint}`;
    } else {
      vIdProofNo.disabled = true;
      vIdProofNo.placeholder = 'Select ID proof type first';
      vIdProofNo.value = '';
    }
    markInvalid('vIdProofNo', false);
  });

  vIdProofNo.addEventListener('input', () => {
    const rule = idProofRules[vIdProof.value];
    if(rule && rule.mode === 'numeric'){
      vIdProofNo.value = vIdProofNo.value.replace(/\D/g, '').slice(0, rule.maxlength);
    } else if(rule){
      vIdProofNo.value = vIdProofNo.value.replace(/[^a-zA-Z0-9]/g, '').toUpperCase().slice(0, rule.maxlength);
    }
    if(vIdProofNo.classList.contains('invalid')) validateForm();
  });

  function isIdProofNoValid(){
    const rule = idProofRules[vIdProof.value];
    if(!rule) return false;
    const val = vIdProofNo.value.trim();
    if(rule.mode === 'numeric') return new RegExp(`^\\d{${rule.maxlength}}$`).test(val);
    return val.length >= 4 && val.length <= rule.maxlength;
  }
  ...
  restrictToAlpha($('#vName'));
  restrictToAlpha($('#vMeetName'));
  ...
  const fieldIds = ['vName','vPhone','vIdProof','vIdProofNo','vMeetName','vRoom','vPersons','vPurpose','vDate','vTimeIn'];
  ...
  function validateForm(){
    ...
    if(!/^[A-Za-z][A-Za-z\s.]{1,49}$/.test(name)){ markInvalid('vName', true); valid = false; } else { markInvalid('vName', false); }
    ...
    if(!isIdProofNoValid()){ markInvalid('vIdProofNo', true); valid = false; } else { markInvalid('vIdProofNo', false); }

    const meetName = $('#vMeetName').value.trim();
    if(!/^[A-Za-z][A-Za-z\s.]{1,49}$/.test(meetName)){ markInvalid('vMeetName', true); valid = false; } else { markInvalid('vMeetName', false); }
  ...
  form.addEventListener('submit', (e) => {
    ...
    const entry = {
      ...
      idProofNo: $('#vIdProofNo').value.trim(),
    ...
    list.forEach(v => {
      ...
      tr.innerHTML = `
        <td data-label="Visitor">
          ...
          <div class="vsub">${escapeHtml(v.idProof || '—')}${v.idProofNo ? ' · ' + escapeHtml(v.idProofNo) : ''}</div>
        </td>
