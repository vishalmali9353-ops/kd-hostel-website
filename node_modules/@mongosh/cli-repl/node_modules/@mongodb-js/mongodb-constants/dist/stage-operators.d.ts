...
declare const VECTOR_SEARCH_AUTO_EMBED_STAGE: {
    name: string;
    value: string;
    label: string;
    outputStage: boolean;
...
declare const STAGE_OPERATOR_NAMES: ("$count" | "$addFields" | "$bucket" | "$bucketAuto" | "$changeStream" | "$changeStreamSplitLargeEvent" | "$collStats" | "$currentOp" | "$densify" | "$documents" | "$facet" | "$fill" | "$geoNear" | "$graphLookup" | "$group" | "$indexStats" | "$limit" | "$listLocalSessions" | "$listSearchIndexes" | "$lookup" | "$match" | "$merge" | "$out" | "$project" | "$rankFusion" | "$redact" | "$replaceRoot" | "$replaceWith" | "$rerank" | "$sample" | "$score" | "$scoreFusion" | "$search" | "$searchMeta" | "$set" | "$setWindowFields" | "$skip" | "$sort" | "$sortByCount" | "$unionWith" | "$unset" | "$unwind" | "$vectorSearch")[];
...
export { STAGE_OPERATORS, STAGE_OPERATOR_NAMES, OUT_STAGES, FULL_SCAN_STAGES, REQUIRED_AS_FIRST_STAGE, VECTOR_SEARCH_AUTO_EMBED_STAGE, };
